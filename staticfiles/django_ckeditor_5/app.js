...
function createEditors(element = document.body) {
    ...
    allEditors.forEach(editorEl => {
        ...
        if (editorEl.nextSibling && editorEl.nextSibling.nodeType == Node.TEXT_NODE && editorEl.nextSibling.textContent.trim() === '') {
        ...
        const configElement = element.querySelector(`#${script_id}-span`);
        if (!configElement) {
            console.error(`Missing config element for editor: ${editorEl.id}`);
            return;
        }
